...
def gold_room():
    ...
    if (0 in next) or (1 in next):
        ...
        dead("인간이여, 숫자를 쓰는 법부터 배우세요.")

    if how_much < 50:
        print "좋아, 욕심 부리지 않는군요. 당신이 이겼습니다!"
        exit(0)
    else:
        dead("욕심쟁이 얼간이 같으니!")
...
def bear_room():
    ...
    while True:
        ...
        if next == "꿀 뻿기":
            dead("곰이 당신을 쳐다보더니 목이 떨어져라 따귀를 날렸습니다.")

        elif (next == "곰 놀리기") and not bear_move:
            print "곰이 문에서 비켜섰습니다. 이제 나갈 수 있습니다."
            bear_move = True

        elif (next == "곰 놀리기") and bear_move:
            dead("곰이 머리 끝까지 열받아 당신의 다리를 씹어먹습니다.")

        elif (next == "문 열기") and bear_move:
            gold_room()
        else:
            print "무슨말이야?"
